...
<head>
    ...
    grobots_norobot(); robots_norobot();
    # Load css
...
<body>
    ...
    <div class="padding-content display-block">
                ...
                if ($mysqli->connect_errno != 0) display_alert("Lỗi khi kết nối đến MySQL, lỗi {$mysqli->connect_error}", LAUTH_ALERT_ERROR);
                    ...
                    $file = lauth_files_create(LAUTH_FILE_CONFIG, join("\n",
                        [
                            ...
                            "define('LAUTH_ADMIN_PASSWORD', '{$_POST['admin-password']}');"
                        ])
                    );

                    if (!$file) { display_alert("Không thể tạo hoặc ghi tệp thiết lập", LAUTH_ALERT_ERROR); }
                    else {
                        ...
                        else { display_alert("Không thể tạo bảng cài đặt của LAuth, lỗi: " . lauth_mysql_last_error(lauth::$_MYSQL), LAUTH_ALERT_ERROR);  }
                    }
                }
            }
        ...
        <div class="container bg-white bs-border-box p-3 animated slideInUp mobile-w-100" id="setup-box">
            ...
            <i style="<?php if (is_valid_php_version(LAUTH_PHP_VERSION_REQUEST)) echo "color:#76ecb0;"; else echo "color:#ec7676;"; ?>">Yêu cầu <b>PHP <?php echo LAUTH_PHP_VERSION_REQUEST ?></b> trở lên. Bạn đang ở phiên bản <?php echo phpversion() ?></i>
            <p>Cảm ơn bạn đã sử dụng LAuth. Để hoàn tất việc sử dụng, hãy làm đủ các bước và điền đầy đủ thông tin bên dưới. <b>LAuth sẽ sử dụng dữ liệu từ cơ sở dữ liệu của AuthMe</b> để hoạt động. Vì thế bạn hãy chắc chắn rằng máy chủ của bạn có sử dụng AuthMe.</p>
            <p>Ngoài ra LAuth còn hỗ trợ rất nhiều thứ khác nữa nhưng đó là trong tương lai. Nếu bạn muốn đóng góp tài nguyên cho LAuth, bạn cũng có thể phát triển module cho LAuth</p>
            <ul>
                <li><a class="bold" href="https://www.spigotmc.org/resources/authmereloaded.6269/">Tải về AuthMeReloaded</a></li>
            </ul>
            <form action="" method="post" class="form mt-2" role="form">
                <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="server-name" title="Tên máy chủ" placeholder="Tên máy chủ..." required value="<?php if (isset($_POST['server-name'])) echo $_POST['server-name']; ?>">
                    </div>
                    <div class="form-group">
                        <label for="server-url">Địa chỉ trang web</label>
                        <input class="form-control" type="url" name="server-url" title="Địa chỉ trang web"  placeholder="Địa chỉ của trang web..." required value="<?php if (isset($_POST['server-url'])) $_POST['server-url']; ?>">
                        <small class="form-small-text">Địa chỉ máy chủ như http://example.com/</small>
                    </div>
                    <div class="form-group">
                        <label for="admin-username">Tài khoản quản trị</label>
                        <input class="form-control" type="text" name="admin-username" title="Tên tài khoản quản trị"  placeholder="Tên tài khoản quản trị..." required value="<?php if (isset($_POST['admin-username'])) echo $_POST['admin-username']; ?>">
                    </div>
                    <div class="form-group">
                        <label for="admin-password">Mật khẩu quản trị</label>
                        <input class="form-control" type="password" name="admin-password" title="Mật khẩu quản trị"  placeholder="Mật khẩu quản trị..." required value="<?php if (isset($_POST['admin-password'])) echo $_POST['admin-password']; ?>">
                    </div>
                </div>
                <div class="form-group">
                    <h3>Cài đặt MySQL</h3>
                    <div class="form-group">
                        ...
                        <div class="form-inline">
                            <input class="form-control" type="text" name="mysql-host" title="Địa chỉ MySQL" placeholder="localhost..."  required value="<?php if (isset($_POST['mysql-host'])) $_POST['mysql-host']; else echo "localhost"; ?>" style="width: 70%;">
                            <input class="form-control" type="text" name="mysql-port" title="Cổng MySQL" placeholder="Cổng"  required value="<?php if (isset($_POST['mysql-port'])) $_POST['mysql-port']; else echo "3306" ?>" style="width: 30%">
                        </div
                    </div>
                    <div class="form-group">
                        <label for="mysql-username">Tài khoản MySQL</label>
                        <input class="form-control" type="text" name="mysql-username" title="Tài khoản MySQL" placeholder="root..."  required value="<?php if (isset($_POST['mysql-username'])) echo $_POST['mysql-username']; ?>">
                    </div>
                    <div class="form-group">
                        <label for="mysql-password">Mật khẩu MySQL</label>
                        <input class="form-control" type="password" name="mysql-password" title="Mật khẩu MySQL" placeholder="Mật khẩu MySQL..."  required>
                    </div>
                    <div class="form-group">
                        <label for="mysql-database">Tên cơ sở dữ liệu MySQL</label>
                        <input class="form-control" type="text" name="mysql-database" title="Tên CSDL MySQL" placeholder="Tên CSDL MySQL..." required value="<?php if (isset($_POST['mysql-database'])) $_POST['mysql-database']; ?>">
                    </div>
                </div>
